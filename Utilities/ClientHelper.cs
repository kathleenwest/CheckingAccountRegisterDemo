	...
	public static class ClientHelper
	{
            ...
            string uriFormat, params object[] values)
		{
			...
			try
			{
                ...
                if (uriFormat == String.Empty)
                {
                    request = new HttpRequestMessage(HttpMethod.Get, String.Empty);
                }
                else
                {
                    request = new HttpRequestMessage(HttpMethod.Get, string.Format(uriFormat, values));
                }
			...
			{
				System.Reflection.MethodBase mb = System.Reflection.MethodBase.GetCurrentMethod();
				System.Diagnostics.Debug.WriteLine(ex.Message, string.Format("ClientHelper{0}.{1}.{2}", mb.DeclaringType.Namespace, mb.DeclaringType.Name, mb.Name));
			}
			return result;
		}
